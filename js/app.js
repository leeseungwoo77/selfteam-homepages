...
function withOverride(section) {
  ...
  if (!o) return section;
  return { ...section, label: o.label || section.label, group: o.group || section.group, color: o.color || section.color, order: o.order ?? section.order };
}
...
function renderAllMenuList() {
  ...
  const allBuiltIn = SECTIONS.map((s, i) => ({ ...s, order: s.order ?? i })).map(withOverride).map(s => ({ ...s, isCustom: false }));
  const allFolders = state.customFolders.map(f => ({ ...withOverride(folderToSection(f)), isCustom: true, folderDocId: f.id }));
  const allItems = [...allBuiltIn, ...allFolders].sort((a, b) => {
