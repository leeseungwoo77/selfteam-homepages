...
### 업무 트레킹 (사용자 정의 폴더 양식)
"지점 · 팀원 관리 → 전체 메뉴 관리"에서 폴더의 "양식 종류"를 **업무 트레킹**으로 바꾸면, 그 폴더가 아래와 같은 전용 화면으로 바뀝니다.
- 업무를 하나 등록하면(업무명 · 마감기한 · 지시 내용) 지점마다 체크박스 한 칸씩 생깁니다.
- 팀장이 그 지점 원장님께 지시를 전달했으면 체크 → 지점별 완료율과 D-day가 자동 계산됩니다.
- 상단 요약 카드에서 전체 업무 수 · 평균 지시 완료율 · 전 지점 완료 건수 · 마감 지난 미완료 건수를 한눈에 볼 수 있습니다.
- 작성 권한을 "전원 작성 가능"으로 바꾸면 각 지점 담당 팀원도 자기 지점 체크박스를 직접 누를 수 있습니다(기본값은 팀장만).
- 업무 트레킹 표에는 "본사"와 "진학중계" 지점은 표시되지 않고, 완료율·통계도 이 두 지점을 뺀 나머지 지점 기준으로 계산됩니다. (`js/app.js`의 `TASK_TRACKING_EXCLUDED_BRANCHES` 배열에서 조정 가능)

### 지표 분석 - 분석 기록
지표 분석 화면 상단 탭에 **분석 기록**이 추가되었습니다. 지표를 보다가 눈에 띈 내용이나 원장님과 나눌 이야기를 지점·연도·월별로 남겨두는 메모장입니다.
- "요약 대시보드"에서 지점·연도·월을 고른 뒤 "이 조건으로 기록 남기기"를 누르면, 그 조건이 그대로 채워진 채로 기록 작성창이 열립니다.
- 팀장/뷰어는 전체 지점 기록을 볼 수 있고, "전체"로 남긴 기록도 쓸 수 있습니다. 원장님(팀원)은 자기 지점 기록과 "전체" 기록만 볼 수 있고, 자기가 쓴 기록만 수정·삭제할 수 있습니다.
- 이 기능을 쓰려면 Firestore 규칙에 `metricAnalysisNotes` 컬렉션 규칙이 추가돼 있어야 하니, 이번에 갱신된 `firestore.rules` 파일 전체를 Firebase 콘솔에 다시 게시해주세요.
